...
def load_transactions(path: Path = DATA_PATH) -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Dataset not found: {path}. Place the existing transaction dataset there or change DATA_PATH in src/config.py.")
    suffix = path.suffix.lower()
    if suffix == ".csv": df = pd.read_csv(path)
    elif suffix in {".parquet", ".pq"}: df = pd.read_parquet(path)
    elif suffix in {".pkl", ".pickle"}: df = pd.read_pickle(path)
    ...
    if df[TIMESTAMP].isna().any(): raise ValueError(f"{TIMESTAMP} contains invalid timestamps.")
