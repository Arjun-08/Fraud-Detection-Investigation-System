...
from .config import (
    ARTIFACT_DIR, MAX_REVIEW_RATE,
    COST_FALSE_NEGATIVE, COST_FALSE_POSITIVE, COST_REVIEW
)
...
def optimize_threshold(y_valid, p_valid):
    ...
    The cost coefficients are project configuration values, not universal
    financial assumptions.
    """
    print("[THRESHOLD] Searching validation thresholds...")

    candidates = []
    for threshold in np.linspace(0.01, 0.99, 197):
        metrics = evaluate_at_threshold(y_valid, p_valid, threshold)
        if metrics["review_rate"] <= MAX_REVIEW_RATE:
            metrics["business_cost"] = float(
                business_cost(y_valid, p_valid, threshold)
            )
            candidates.append(metrics)

    if not candidates:
        raise RuntimeError("No threshold satisfies MAX_REVIEW_RATE.")

    best = min(candidates, key=lambda x: x["business_cost"])

    print(
        f"[THRESHOLD] Selected {best['threshold']:.3f}; "
        f"F1={best['f1']:.4f}; "
        f"review_rate={best['review_rate']:.4%}; "
        f"cost={best['business_cost']:.2f}"
    )
    return best
